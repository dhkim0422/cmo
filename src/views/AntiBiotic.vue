<template>
    <div class="container">
      <section class="section-search mT-60">
        <h3 class="h3 title-section mB-30"><img src="../images/arg-database-ico-s.png" alt="BLAST">항생제</h3>
        <table class="table-view">
          ...
          <tbody>
            <tr>
              ...
              <td class="td-components"><input type="text" v-model="params.searchAbClass" class="form-control" placeholder="e.g. vana"></td>
              <th class="th-title">항생제</th>
              <!--<td class="text-red">vana</td>-->
              <td class="td-components"><input type="text" v-model="params.searchAb" class="form-control" placeholder="e.g. vana"></td>
            </tr>
          </tbody>
        </table>
          ...
          <table class="table-list">
            ...
            <thead>
              <tr>
                ...
                <th class="text-center" scope="col">#. 치료에 사용되는 질병</th>
                <th class="text-center" scope="col">#. 항생제 내성 Subclass</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="row in resultList" :key="row.abId">
                ...
                <td> {{ row.abClass }}</td>
                <td><a @click="gotoDetail(row)" class="a-link pointer">{{ row.ab }}</a></td>
                <td>{{row.abDesc}}</td>
                <td class="ta-r">{{ row.countDs | money}}</td>
                <td class="ta-r">{{ row.countArtp | money}}</td>
              </tr>
            </tbody>
          </table>
...
export default {
  ...
  methods: {
    ...
    gotoRegister() {
      this.$router.push({name: 'antiBioticRegister'})
    },
  }
}
</script>
