<template>
    <div class="container">
        ...
        <div class="filter-group">
            <div class="group-item">
                <div class="custom-checkbox custom-checkbox-all">
                    <input type="checkbox" id="chkPrjAll" class="custom-control-input"
                           ng-model="modelHandler.checkedAll"
                           ng-change="modelHandler.selectAll(modelHandler.checkedAll)" aria-invalid="false">
                    <label class="custom-control-label" for="chkPrjAll"><span class="sr-only">전체선택</span></label>
                </div>
                <div class="info">
                    전체<span class="num ng-binding">{{this.resultList.data.total}}</span>개,
                    현재 페이지<span class="num ng-binding">1</span> /
        ...
        <div v-for="result in resultList.data.list" class="data-card">
            <div class="card-header">
                <div class="block-group">
                    <div class="block">
                        <div class="custom-checkbox" style="width: 270px">
                            <input type="checkbox" name="chkPrj"
                                   class="custom-control-input"
                                   aria-invalid="false"/>
                            <label class="custom-control-label" for="chkPrj2"><i class="xi-receipt"></i>{{result.accession}}</label>
                        </div>
                    </div>
                    <div class="block" style="width: 800px; margin-left: 55px">
                        <a role="button" class="link-text ng-binding" @click="onClickDetailLink(result);"
                           title="상세보기">
                            {{result.program}} <i class="xi-angle-right"></i>
                        </a>
                    </div>
                </div>
            </div>
